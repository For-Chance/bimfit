...
def mid():
    ...
    def confirmFinished(orderId, status):
        if status == 2:
            cursor.execute(
                "update `order` set status = 3 where orderId = {orderId};".format(
                    orderId=orderId
                )
            )
        else:
            cursor.execute(
                "update `order` set status = 4 where orderId = {orderId};".format(
                    orderId=orderId
                )
            )
        clear(scope="mid")
    ...
    def getConfirmFinishedBtn(eachOrder):
        return put_button(
            "确认结束",
            onclick=lambda: confirmFinished(eachOrder[0], eachOrder[3]),
            color="light",
            disabled=False if eachOrder[3] < 3 else True,
        )
    ...
    # 通过sql获得页面所需的数据
    cursor.execute(
        "select orderId, username as decoratorName, response, `status`, bimModelFilePath from `order`, `user` where `order`.userId = {id} and `order`.decoratorId = `user`.userId;".format(
            id=id_query
        )
    )
    ...
    for eachOrder in orders:
        ...
        btns = put_column(
            [
                put_markdown(
                    "装修公司：**{decoratorName}**".format(
                        decoratorName=eachOrder[1]
                ...
                put_row(
                    [
                        ...
                        getConfirmFinishedBtn(eachOrder),
                    ]
                ),
            ],
            size="35% 65%",
